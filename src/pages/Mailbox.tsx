...
const Mailbox: React.FC = () => {
  ...
  useEffect(() => {
   ...
   //     // Background refresh logic removed
   //   }, 30000);
  ...
  const loadEmailsForView = async (view = currentView, pageToken?: string | null, forceRefresh = false) => {
    ...
    try {
      ...
      if (data.conversations) {
        ...
        // PERFORMANCE OPTIMIZATION: Skip expensive individual thread fetching
        // Since we now load 200 emails and have comprehensive caching, 
        // the missing threads check causes too many individual API calls
        console.log('Skipping individual thread fetching for better performance');
      }
